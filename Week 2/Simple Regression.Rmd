...
a + geom_point(size = 2) + geom_smooth(method = lm)
```
...
```{r}
ggplot(data = mlb11, aes(x = homeruns, y = runs)) + geom_point() + geom_smooth(method=lm)
```
...
m12 <- lm(runs ~ bat_avg, data = mlb11)
summary(m12)
```
```{r}
...
```
```{r}
ggplot(data = m12, aes(sample = .resid)) +
  stat_qq()
```
...
m12 <- lm(runs ~ new_obs, data = mlb11)
summary(m12)
```
```{r}
ggplot(data = m12, aes(x = .fitted, y = .resid)) +
